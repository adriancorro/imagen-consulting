...
router.post("/sign-in", async function (req, res) {
    ...
    try {
      ...
      // guardaremos el toke en en una db para verificar en cualquier momento y parte de la page si el login es true
      let pushToken = {
        user: user[0].email,
        token: jwtToken
      }
       usersDb.push(pushToken);  // we add newUser to usersDb array
       await fs.writeFileSync('./database/db.json', JSON.stringify(usersDb));
